...
1. For ordinary next-card selection, prefer `claim_next_task` with the same pinned `projectId` and this chat's stable session identity. On the first loop claim set `boardLoopRequested=true`; when the user requested a parent/program also pass `requestedTaskId`. Default to `selectionPolicy=todo-only`. Set `selectionPolicy=include-backlog` only for explicit backlog intent. Once a loop is active, reuse its persisted policy by default. A later worker may explicitly and monotonically upgrade the same active loop from `todo-only` to `include-backlog` only when fresh user intent explicitly authorizes backlog; omitted/default input never widens authority and an active `include-backlog` loop is never downgraded.
...
9. For continue-until-terminal work, the execution specialist should make the final authoritative verification request the autonomous terminal handoff: choose the conventional commit message first and call `run_project_command` with a non-empty `autonomousTail.commitMessage`; the legacy `enabled=true` opt-in is no longer required. Once accepted with authoritative GREEN evidence, DevFlow owns deterministic task-owned commit, integration/finalization, required post-integration verification, cleanup, and DONE. If GREEN exists but commit intent is missing, follow `get_execution_continuation` and call `continue_task_execution_tail` with only the missing commit intent instead of rerunning equivalent verification. Do not manually orchestrate those post-GREEN steps unless structured continuation explicitly requires fallback/recovery; in that fallback path, use `finalize_task_workspace` rather than treating GREEN or a clean commit as terminal. Ordinary verification-only or investigation calls remain non-autonomous.
