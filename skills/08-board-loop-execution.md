...
9. For continue-until-terminal work, the execution specialist should make the final authoritative verification request the autonomous terminal handoff: choose the conventional commit message first and call `run_project_command` with `autonomousTail.enabled=true` and that non-empty `commitMessage`. Once accepted with authoritative GREEN evidence, DevFlow owns deterministic task-owned commit, integration/finalization, required post-integration verification, cleanup, and DONE. Do not manually orchestrate those post-GREEN steps unless no autonomous tail was admitted or structured continuation explicitly requires fallback/recovery; in that fallback path, use `finalize_task_workspace` rather than treating GREEN or a clean commit as terminal. Ordinary verification-only or investigation calls remain non-autonomous.
