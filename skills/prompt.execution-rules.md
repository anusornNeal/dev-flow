...
- For file edits, prefer Steno Edit as the default path: read the target with `read_local_file(includeFileRef=true)`, prepare with `prepare_compact_edit`, then apply the returned plan with `apply_prepared_edit`.
