...
export default function JsonTemplateModal({ onClose }: JsonTemplateModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in select-text">
      ...
      <div className="bg-[#fcfaf5] border border-[#ebdcb9] w-full max-w-2xl rounded-3xl shadow-2xl relative z-10 overflow-hidden flex flex-col justify-between font-sans max-h-[85vh]">
        
        {/* Header toolbar */}
        <div className="p-5 border-b border-[#ebdcb9] bg-[#ebdcb9]/40 flex items-center justify-between font-mono text-[#5c493c]">
          <div className="flex items-center gap-2">
            <FileCode size={18} className="text-[#bf8a50]" />
        ...
        {/* Tab Buttons bar */}
        <div className="bg-[#f5eedf]/60 border-b border-[#ebdcb9] px-4 pt-2 flex items-center gap-2">
          <button
            onClick={() => setActiveTab('schema')}
            className={`px-4 py-2 text-xs font-extrabold font-mono border-t border-x rounded-t-xl transition-all cursor-pointer ${
              activeTab === 'schema'
                ? 'bg-[#fcfaf5] border-[#ebdcb9] text-[#784d21] -mb-[1px]'
                : 'bg-transparent border-transparent text-[#9e8470] hover:text-[#5c493c]'
            }`}
          >
            📄 JSON Schema Spec
          </button>
        ...
        {/* Content Body */}
        <div className="p-6 overflow-y-auto scrollbar-thin text-xs text-[#5c493c] flex-1">
          {activeTab === 'schema' ? (
            <div className="space-y-4">
              <div className="space-y-2">
                <h3 className="text-xs font-bold text-[#3c2a1a] flex items-center gap-2">
                  <span className="w-1.5 h-3 bg-[#ebdcb9] rounded-full inline-block" />
                  โครงสร้างข้อมูล JSON สำหรับนำเข้า/สำรองข้อมูล (Import Template)
                </h3>
                <p className="text-[11px] text-[#7a6455] leading-relaxed font-sans">
                  คุณสามารถแก้ไขรายการงานแบบกลุ่ม (Batch) เพื่อจัดการเอกสารล่วงหน้า บันทึกเป็นไฟล์ <code className="bg-[#f5eedf] px-1.5 py-0.5 rounded border border-[#ebdcb9] font-mono text-[#aa7233] text-[10px]">.json</code> แล้วนำเข้าผ่านปุ่ม <strong className="text-[#3c2a1a]">Restore</strong> ด้านบนเพื่อเชื่อมโยงกับ API ทันที
                </p>
              </div>

              <div className="space-y-1.5 bg-[#ffffff] border border-[#ebdcb9] rounded-2xl overflow-hidden shadow-2xs">
                <div className="bg-[#f5eedf]/60 px-4 py-2 border-b border-[#ebdcb9] flex justify-between items-center text-[10px]">
                  <span className="text-[#715c4d] font-mono font-bold">template-backlog.json</span>
                  <div className="flex items-center gap-3">
                    <button
                      type="button"
                      onClick={handleDownload}
                      className="hover:text-[#3a2010] bg-white border border-[#ebdcb9] px-2.5 py-1 rounded-xl cursor-pointer text-[#715c4d] font-semibold flex items-center gap-1 transition-all"
                    >
                    ...
                    <button
                      ...
                      className={`border px-2.5 py-1 rounded-xl cursor-pointer font-semibold flex items-center gap-1 transition-all ${
                        copied && copiedText === 'schema'
                          ...
                          : 'bg-white border-[#ebdcb9] text-[#715c4d] hover:text-[#3a2010]'
                      }`}
                    >
                      {copied && copiedText === 'schema' ? (
                        <>
                ...
                <pre className="p-4 bg-[#fffdfa] overflow-x-auto text-[11px] leading-relaxed text-[#a46c24] font-mono scrollbar-thin max-h-64 font-semibold">
                  <code>{jsonString}</code>
                </pre>
              </div>

              <div className="border-t border-[#ebdcb9]/60 pt-4 space-y-2 font-sans text-[11px] text-[#7a6455] leading-relaxed">
                <p className="font-mono text-[9px] uppercase tracking-wider text-[#8a6e5a] font-bold">ฟิลด์ที่สำคัญประกอบด้วย:</p>
                <ul className="list-disc list-inside space-y-1 pl-1">
                  <li><strong className="font-mono text-[10.5px] text-[#3c2a1a]">id</strong>: คีย์หลักระบุแต่ละงาน ต้องไม่ซ้ำกัน</li>
                  <li><strong className="font-mono text-[10.5px] text-[#3c2a1a]">repo</strong>: บังคับเชื่อมโยง URL ของ Repository เช่น <code className="font-mono bg-[#f5eedf] px-1 text-[10px]">"https://github.com/user/repo"</code> (*Required* สำหรับการสร้างใหม่)</li>
                  <li><strong className="font-mono text-[10.5px] text-[#3c2a1a]">status</strong>: สถานะบอร์ด <code className="font-mono bg-[#f5eedf] px-1 text-[10px]">"backlog" | "todo" | "in-progress" | "ready-for-review" | "done"</code></li>
                  <li><strong className="font-mono text-[10.5px] text-[#3c2a1a]">priority</strong>: ระดับความเร่งด่วน <code className="font-mono bg-[#f5eedf] px-1 text-[10px]">"low" | "medium" | "high"</code></li>
                  <li><strong className="font-mono text-[10.5px] text-[#3c2a1a]">targetFiles</strong>: รายชื่อพาธไฟล์ที่ระบบเกี่ยวข้อง</li>
                  <li><strong className="font-mono text-[10.5px] text-[#3c2a1a]">checklist</strong>: โครงสร้าง Mini-Tasks ย่อยภายในแต่ละตั๋ว</li>
                  <li><strong className="font-mono text-[10.5px] text-[#3c2a1a]">agent</strong>: เอเจนต์ที่รับผิดชอบ <code className="font-mono bg-[#f5eedf] px-1 text-[10px]">"Codex" | "Antigravity" | "Claude"</code></li>
                  <li><strong className="font-mono text-[10.5px] text-[#3c2a1a]">model</strong>: ชื่อตัวแปรโมเดล AI Spec เช่น คู่ขนานตามโมเดลที่เลือก</li>
                  <li><strong className="font-mono text-[10.5px] text-[#3c2a1a]">effort</strong>: ระดับพละกำลังความเพียรประมวลผล <code className="font-mono bg-[#f5eedf] px-1 text-[10px]">"low" | "medium" | "high" | "xhigh"</code></li>
                  <li><strong className="font-mono text-[10.5px] text-[#3c2a1a]">reasoning</strong>: เหตุผลหรือบริบทที่มาของงาน</li>
                  <li><strong className="font-mono text-[10.5px] text-[#3c2a1a]">acceptanceCriteria</strong>: เกณฑ์การตรวจรับงาน (Acceptance Criteria)</li>
                  <li><strong className="font-mono text-[10.5px] text-[#3c2a1a]">verification</strong>: ขั้นตอนการตรวจสอบหรือทดสอบว่าเสร็จสมบูรณ์</li>
                  <li><strong className="font-mono text-[10.5px] text-[#3c2a1a]">repoContext</strong>: บริบทเพิ่มเติมเกี่ยวกับ Repository ที่ใช้</li>
                  <li><strong className="font-mono text-[10.5px] text-[#3c2a1a]">jiraKey</strong>: รหัสบั๊ก/งานบน Jira (เช่น QCA-3314)</li>
                  <li><strong className="font-mono text-[10.5px] text-[#3c2a1a]">repo</strong>: ลิงก์ไปยัง Repository ที่เกี่ยวข้อง</li>
                  <li><strong className="font-mono text-[10.5px] text-[#3c2a1a]">sourceUrl</strong>: URL อ้างอิงต้นทางของตั๋วงาน</li>
                </ul>
              </div>
            </div>
          ) : (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                {apiSpecs.map((api, idx) => {
                  ...
                  return (
                    <div key={idx} className="bg-white border border-[#ebdcb9] rounded-2xl overflow-hidden shadow-3xs flex flex-col">
                      {/* Sub-header with Method & Path */}
                      <div className="bg-[#fcfaf5] border-b border-[#ebdcb9] px-4 py-2.5 flex items-center justify-between font-mono">
                        <div className="flex items-center gap-2.5">
                          <span className={`px-2 py-0.5 rounded-lg border text-[9.5px] font-black tracking-wide ${methodColors[api.method] || 'bg-gray-100'}`}>
                            {api.method}
                          </span>
                          <span className="text-[11.5px] font-bold text-[#3c2a1a]">{api.path}</span>
                        </div>
                        <button
                          type="button"
                          ...
                          className={`text-[9.5px] border px-2 py-0.5 rounded-lg font-bold cursor-pointer transition-colors ${
                            copied && copiedText === `example-${idx}`
                              ...
                              : 'bg-white border-[#ebdcb9] text-[#7a6455] hover:text-[#3c2a1a]'
                          }`}
                        >
                      ...
                      <div className="p-4 space-y-3 font-mono text-[10.5px]">
                        <div>
                          <p className="text-[#8c7463] font-bold text-[9px] uppercase tracking-wider mb-0.5">Description:</p>
                          <p className="text-[#3c2a1a] font-sans text-xs leading-relaxed">{api.description}</p>
                        </div>

                        {api.payload && (
                          <div>
                            <p className="text-[#8c7463] font-bold text-[9px] uppercase tracking-wider mb-1">Request Body (JSON):</p>
                            <pre className="p-2.5 bg-[#fefdfb] border border-[#e5d4bb]/70 rounded-xl overflow-x-auto text-[10px] text-[#aa7233] leading-relaxed max-h-36 scrollbar-thin">
                        ...
                        <div>
                          <p className="text-[#8c7463] font-bold text-[9px] uppercase tracking-wider mb-0.5">Expected Response:</p>
                          <p className="text-[#554030] bg-[#fffcf7] px-2.5 py-1 rounded-lg border border-[#f5ecd4] font-semibold">{api.response}</p>
                        </div>

                        <div>
                          <p className="text-[#8c7463] font-bold text-[9px] uppercase tracking-wider mb-1">Code Pattern Example (Fetch JS):</p>
                          <pre className="p-2.5 bg-[#1e293b] text-[#38bdf8] rounded-xl overflow-x-auto font-mono text-[10px] leading-relaxed">
        ...
        {/* Footer */}
        <div className="p-4 bg-[#ebdcb9]/40 border-t border-[#ebdcb9] flex justify-end text-xs font-mono">
          <button
            type="button"
            onClick={onClose}
