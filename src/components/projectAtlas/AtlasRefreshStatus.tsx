...
export function AtlasRefreshStatus({ stale, status, scanState = 'idle', message }: AtlasRefreshStatusProps) {
  ...
  return (
    <span className="h-9 rounded-xl border border-[#e5d4bb] bg-[#fff7eb] px-3 py-2 text-[10px] font-mono font-black uppercase text-[#9a5b13] dark:border-[#584a3b] dark:bg-[#3a2f26] dark:text-[#f3eadf]" title={detail}>
      {label}
    </span>
  );
}
