export function buildChatGptStarterPrompt() {
  return [
    ...
    '2. Use get_project_start_context, get_repo_inspection_index, list_local_files, search_local_files, and read_local_file only when the repo context bundle is unavailable or insufficient.',
    '3. Use list_projects only when the target project is unclear.',
    '4. For Jira-originated card authoring, call get_jira_authoring_bundle before individual jira_get_* proxy tools.',
    '5. For card authoring fallback searches, call get_repo_inspection_index with screen/string/Jira terms before broad file search.',
    '6. Use read_local_file with startLine/endLine/maxBytes before remote GitHub/Jira reads unless the user explicitly asks for remote data.',
    '7. Prefer get_agent_task_context for a specific card, and list_tasks/search_tasks for finding cards.',
    '',
