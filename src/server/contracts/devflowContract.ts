...
export const DEVFLOW_CONTRACT_VERSION = '2026-08-31.1';
...
export const devFlowToolDefinitions: DevFlowToolDefinition[] = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        clientUnavailableToolNames: { type: 'array', maxItems: 64, items: { type: 'string', minLength: 1, maxLength: 160 }, description: 'Exact tool names the prior client knows are not callable despite the advertised server surface.' },
      },
    },
    outputSchema: { type: 'object' },
    lightweight: true,
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        clientUnavailableToolNames: { type: 'array', maxItems: 64, items: { type: 'string', minLength: 1, maxLength: 160 }, description: 'Exact tool names this client knows are not callable despite the advertised server surface.' },
  ...
  },
  {
    ...
    description: 'Read the bounded tail of one durable project-level worker diagnostic log.',
    inputSchema: {
      type: 'object',
      properties: {
        projectId: { type: 'string' },
        workerId: { type: 'string' },
        maxBytes: { type: 'number', minimum: 1, maximum: 524288 },
      },
      required: ['projectId', 'workerId'],
    },
    outputSchema: { type: 'object' },
    lightweight: true,
    buildHttpRequest: (args) => ({ method: 'GET', path: withQuery('/api/worker-logs/read', args) }),
  },
  {
    name: 'list_worker_logs',
    description: 'List bounded durable project-level worker diagnostic logs without reading their contents.',
    inputSchema: {
      type: 'object',
      properties: {
        projectId: { type: 'string' },
    ...
    outputSchema: { type: 'object' },
    lightweight: true,
    buildHttpRequest: (args) => ({ method: 'GET', path: withQuery('/api/worker-logs', args) }),
  },
  {
...
const CODING_PROFILE_TOOLS = new Set([
  ...
  'write_local_file', 'append_worker_log', 'read_worker_log', 'list_worker_logs', 'edit_local_files_batch', 'prepare_compact_edit', 'apply_prepared_edit', 'apply_and_verify', 'delete_local_path', 'move_local_path',
