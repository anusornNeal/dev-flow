...
export const gitToolDefinitions: DevFlowToolDefinition[] = [
  ...
  {
    name: 'commit_task_owned_changes', executionPolicy: { mode: 'job', jobKind: 'repo-command' }, description: 'Commit only files owned by the selected task execution session. Ordinary commits require fresh verification; explicit emergency preserveVerificationDebt mode is limited to proven verification infrastructure failures and never finalizes the task.',
    inputSchema: { type: 'object', properties: { ...projectIdentifierProperties, taskId: { type: 'string', description: 'DevFlow task id/displayId whose execution ownership defines commit scope.' }, message: { type: 'string', description: 'Conventional commit input such as fix(scope): description. DevFlow normalizes it through the task/project commit policy and adds the authoritative task/ticket prefix by default.' }, dryRun: { type: 'boolean', description: 'Preview the scoped commit without creating it.' }, preserveVerificationDebt: { type: 'boolean', description: 'Emergency-only: commit owned files while preserving failed infrastructure verification as explicit outstanding debt.' }, emergency: { type: 'boolean', description: 'Explicit operator authorization required for preserveVerificationDebt.' }, reason: { type: 'string', description: 'Required audit reason for preserveVerificationDebt.' }, actorLabel: { type: 'string', description: 'Required human/operator label for preserveVerificationDebt audit evidence.' }, expectedCandidateId: { type: 'string', description: 'Optional expected verification candidate identifier recorded with verification debt.' } }, required: ['taskId', 'workspaceId', 'message'] },
