  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        partitionCount: { type: 'number', description: 'Optional deterministic worker partition count. Supply together with partitionIndex.' },
        partitionIndex: { type: 'number', description: 'Optional zero-based deterministic worker partition index. Supply together with partitionCount.' },
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        partitionCount: { type: 'number', description: 'Optional deterministic worker partition count. Must be supplied together with partitionIndex; current server limit is 100.' },
        partitionIndex: { type: 'number', description: 'Optional zero-based deterministic worker partition index. Candidate membership uses the numeric display-id suffix modulo partitionCount.' },
        ...mutationResponseModeProperty,
      },
      required: ['projectId', 'sessionId'],
    },
  ...
  // External/Codex board synchronization is intentionally separate from managed lifecycle movement.
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        worker: { type: 'string', maxLength: 128, description: 'Optional external/local-native worker label for Agent Office visibility. This is replaceable presence metadata, not lifecycle ownership.' },
        action: { type: 'string', enum: ['IMPLEMENT_TASK', 'RESOLVE_FAILURE', 'RESOLVE_CONFLICT', 'REVIEW_TASK', 'INVESTIGATE'], description: 'Optional canonical orchestration action currently handled by the external worker.' },
        resultState: { type: 'string', enum: ['HANDOFF_READY', 'BLOCKED', 'NEEDS_CONTEXT', 'COMPLETE'], description: 'Optional normalized orchestration result. Non-COMPLETE results keep status in-progress so attention can be resumed; COMPLETE requires ready-for-review or done.' },
        contextRef: { type: 'string', maxLength: 1000, description: 'Optional durable context reference for a replacement compatible worker; never a local execution harness path requirement.' },
