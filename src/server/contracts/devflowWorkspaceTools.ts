  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        checks: { type: 'array', description: 'Optional verification evidence preserved truthfully during finalization. Missing, failed, stale, or not-run evidence becomes explicit quality debt rather than a mechanical finalization blocker.', items: { type: 'object', properties: { name: { type: 'string' }, command: { type: 'string' }, targets: { type: 'array', items: { type: 'string' }, description: 'Exact repository-relative verification targets when the preset requires targets.' }, status: { type: 'string', enum: ['passed', 'failed', 'not-run'] }, scope: { type: 'string', enum: ['targeted', 'broad', 'full'], description: 'Verification coverage scope for combined-state finalization.' }, repoRevision: { type: 'string', description: 'Exact integrated Git revision this verification check was run against.' }, summary: { type: 'string' }, output: { type: 'string' }, recordedAt: { type: 'string' } }, required: ['command', 'status'] } },        completedChecklistIds: { type: 'array', maxItems: 100, uniqueItems: true, items: { type: 'string', minLength: 1, maxLength: 200 }, description: 'Optional explicit checklist item ids attested complete by the owning caller. Finalization validates them against the selected task and freezes them into the durable operation; omitted input preserves existing checklist state.' },
