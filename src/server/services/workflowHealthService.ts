...
export function getWorkflowHealth(state: AppState, args: Record<string, any> = {}) {
  ...
  if (failedJobs > 0) recommendations.push('Recent tool jobs include failures; inspect logs/results before retrying broad work.');
  ...
  return {
    ...
    git,
    diagnostics: { queueDepth, failedJobs, staleAgentRuns, duplicateBursts },
    recommendations,
  };
}
