...
function makeDb(root: string, name: string, migrationCount = DEVFLOW_MIGRATIONS.length) {
...
test('creates a checksummed verified snapshot with schema metadata and strips portable secrets', async () => {
  ...
  try {
    ...
    assert.equal(copy.prepare("SELECT value FROM settings WHERE key='githubToken'").get()?.value, '');
    copy.close();
