...
test('devflowContract exposes execute_repo_query_plan as a bounded read-only coding/full job', () => {
  ...
  const stepItems = tool.inputSchema?.properties?.steps?.items;
  assert.ok(Array.isArray(stepItems?.oneOf), 'repo query plan steps should expose operation-specific schema branches');
  const branches = stepItems.oneOf as any[];
  const branchFor = (op: string) => branches.find((branch) => branch?.properties?.op?.const === op);
  const selectBranch = branchFor('select');
  const limitBranch = branchFor('limit');
  const readBranch = branchFor('read_snippets');
  assert.ok(selectBranch);
  assert.equal(selectBranch.additionalProperties, false);
  assert.equal(selectBranch.properties.count, undefined, 'select must not advertise limit-only count');
  assert.ok(selectBranch.properties.fields);
  assert.ok(limitBranch?.properties?.count);
  assert.equal(limitBranch?.properties?.fields, undefined);
  assert.equal(readBranch?.properties?.contextAfter?.maximum, 50);
