...
test('tracker records initialize, tools/list, and tools/call lifecycle phases from real Streamable HTTP requests', async () => {
  ...
  app.post('/mcp', async (req, res, next) => {
    ...
    try {
      await createStatelessMcpHttpHandler(apiBaseUrl, 'full', tracker.hooks)(req, res, next);
    } finally {
  ...
  try {
    await post({ jsonrpc: '2.0', id: 1, method: 'initialize', params: { protocolVersion: '2025-06-18', capabilities: {}, clientInfo: { name: 'monitor-test', version: '1' } } });
    await post({ jsonrpc: '2.0', id: 2, method: 'tools/list', params: {} });
    await post({ jsonrpc: '2.0', id: 3, method: 'tools/call', params: { name: 'devflow_health_check', arguments: {} } });
